...
### เจอ + แก้ไฟล์ขยะปนอยู่ใน vault ระหว่างสำรวจสำหรับปรับหน้าโชว์งานวิจัย/รายงานประเมิน

ระหว่างสำรวจโครงสร้างเอกสาร "general" (งานวิจัย) เพื่อเตรียมปรับหน้าเว็บ เจอ 2 ปัญหา
ไม่เกี่ยวกับ semantic search แต่กระทบคุณภาพ index โดยตรง:

1. **โปรเจกต์โค้ดคนละโปรเจกต์ทั้งหมดถูกก็อปมาไว้ในโฟลเดอร์ vault** — พบโฟลเดอร์ชื่อ
   `human-rights-platform/` (มี `node_modules`, `apps/`, `docs/` ครบ - หน้าตาเป็น
   โปรเจกต์ software แยกต่างหาก) วางอยู่ที่ root ของ vault ตอนแรก parser ไม่มีเงื่อนไข
   ยกเว้นโฟลเดอร์นี้ ทำให้ตอน re-index (ระหว่างทดลองแก้ routing ข้อ 2) ไฟล์
   `.md` ข้างใน 368 ไฟล์ (README ของ npm package ต่างๆ, AGENTS.md, docs/*.md)
   หลุดเข้ามาเป็น "งานวิจัย" ปลอมทั้งหมด (general กระโดดจาก 90 เป็น 452!) —
   แก้โดยเพิ่มเงื่อนไขยกเว้นโฟลเดอร์นี้ใน `parse_file()` โดยตรง
2. **5 กรณีปี 2569 มีไฟล์ซ้ำวางผิดที่** — เจอ `.md` ดิบ (ไม่มี "- บันทึก") วางอยู่ที่
   top-level `ปี 2569/` (โฟลเดอร์ที่เอกสารระบุไว้แล้วว่าใช้เก็บ PDF scan เท่านั้น
   ดู `_find_case_pdf`) ทั้งที่กรณีเดียวกันมีไฟล์ "- บันทึก" ที่ถูกต้องอยู่แล้วใน
   `03 กรณีตรวจสอบ (Case Notes)/ปี 2569/` **ลองแก้ผิดทางก่อน** (route
   top-level "ปี XXXX" ไปเป็น case_note) แต่พบว่าจะชน document_id เดียวกับตัวที่ถูกต้อง
   อยู่แล้ว เสี่ยง silent overwrite ขึ้นกับลำดับ glob() ของ filesystem (ตรวจแล้วว่า
   เกือบเกิดขึ้นจริง) - **แก้ที่ถูกคือ exclude ไฟล์ .md ใน top-level "ปี XXXX" ไปเลย**
   (ซ้ำกับของที่มีอยู่แล้ว ไม่ใช่กรณีที่ขาดหาย)

**ผลหลัง reindex ใหม่**: `general` ลดจาก 90 เหลือ 84 (case_note/topic/situation_report
ไม่เปลี่ยน) รวม 398 เอกสาร (ลดจาก 404 เดิม 6 รายการที่เป็นไฟล์ซ้ำ/ขยะ ไม่ใช่ของ
หายไปจริง) ตรวจแล้วว่าไม่มีเอกสารเนื้อหาจริงหายไปเลย (diff แบบ document_id ต่อ
document_id กับ index เดิมก่อนหน้า) เก็บกวาด `data/nhrc_content/*.txt` ที่ orphan
ไปด้วย (พบว่ามีไฟล์ขยะเก่าติดค้างจากเวอร์ชันก่อนหน้าของ pipeline อีก ~16 ไฟล์ที่เคย
commit ไปแล้วด้วย เช่น Template/README ของ Obsidian เอง)

### บั๊กใหญ่: 32 กรณีร่วม (joint case) หายไปจากระบบทั้งหมด - ไม่ใช่แค่กราฟ

ผู้ใช้แจ้งว่าคลิกโหนดในกราฟแล้วเห็นกรณีที่เกี่ยวข้องน้อยกว่าที่ควร/ดูไม่ค่อยเชื่อมโยงกัน
+ อยากซูม/ค้นหาในกราฟได้ + อยากให้โหนดละเอียดขึ้น ตรวจสอบพบสาเหตุราก:

`_parse_case_note`'s regex เดิม (`^(\d+)-(\d+)[_\s]...`) รองรับแค่ "เลขคดีเดียว-ปี"
(เช่น "128-2563") แต่ในวอลต์มีไฟล์คดีที่สอบสวนร่วมกันหลายเลขคดี ชื่อไฟล์แบบ
"79-80-2568_..." (2 คดี) หรือ "203-214-2564_..." (12 คดีถูกยื่นรวมเป็นเรื่องเดียว)
ซึ่ง regex เดิม**parse ไม่ผ่านเลย** (`re.match` คืน None) ทำให้ไฟล์เหล่านี้หายไปจาก
**ทุกที่ในระบบ** (ค้นหา, Ask NHRC, กราฟ) ไม่ใช่แค่กราฟ - ตรวจนับได้ 32 ไฟล์แบบนี้
ในวอลต์ทั้งหมด

**แก้**: เปลี่ยน regex เป็น `^((?:\d+-)*\d+)-(\d{4})[_\s]...` (จับกลุ่มเลขคดีที่คั่นด้วย
"-" กี่ตัวก็ได้ ก่อนปี 4 หลัก) `case_id` กลายเป็น "79-80-2568" (คงรูปแบบเดิม),
`document_id` เป็น "case_79_80_2568" (แปลง "-" เป็น "_" ให้ตรง convention เดิม)
**ต้องแก้ `CASE_REF_PATTERN` ด้วย** (regex แยกต่างหากที่ใช้สร้าง backlink
topic↔case) เพราะเดิมมันตีความ "79-80-2568" ผิดเป็น case_num="79" ปี="80" (เอาแค่
เลขคดี 2 ตัวแรกโดยไม่รู้จักปี 4 หลัก) ทำให้ case_id ที่สร้างจาก topic backlink
("79-80") กับจาก case_note เอง ("79-80-2568" หลังแก้) **ไม่ตรงกัน** ต้องแก้ทั้งคู่ให้
ใช้ตรรกะเดียวกัน (จับ 4 หลักสุดท้ายเป็นปีเสมอ) ถึงจะ join กันถูก

**ผลหลัง reindex**: case_note 285 → 317 (+32), by-area รวม 294 → 324 (+30, มี 2
กรณีที่ไม่มี topic backlink เลยเลยไม่ได้ area), graph shared-case links 70 → 73
ทดสอบ query กราฟ topic "เสรีภาพชุมนุมแสดงออกสื่อ" ตรงกับที่ตัวไฟล์ topic เขียนไว้เอง
ว่ามี "12 รายการ" ได้ครบ 12 พอดี (ก่อนแก้ได้แค่ 7 เพราะ 5 กรณีร่วมในนั้นถูกกรองทิ้ง)
**ตรวจข้อมูลส่วนบุคคลของ 32 ไฟล์ที่กู้คืนมาแล้ว** (ไม่เคยถูกตรวจมาก่อนเพราะไม่เคย
parse ผ่านมาก่อนเลย) - เจอ 1 hit เดียว เป็น false positive แบบเดียวกับที่เจอไปแล้ว
("นายจ้าง") ปลอดภัย - รัน embedding script เพิ่มให้ 32 กรณีนี้ด้วยแล้ว (semantic
search จะหาเจอด้วย ไม่ใช่แค่ keyword)

**ยังไม่ทำ**: ซูม/pan ในกราฟ, ค้นหาในกราฟ, เพิ่มความละเอียดของโหนด (ขอบเขต Part
ที่ผู้ใช้ยังไม่ได้ยืนยันสเกลของงาน - ตัวบั๊กข้อมูลหายที่เป็นสาเหตุหลักของ "เห็น
ความสัมพันธ์น้อย" แก้ไปแล้ว ส่วนฟีเจอร์ interactive เพิ่มเติมยังรอคุยขอบเขตกับผู้ใช้)
...
## ขยายคลังความรู้ (กฎหมายไทย + ตราสารสิทธิมนุษยชนระหว่างประเทศ) — 7 ส.ค. 2569

ผู้ใช้ขอให้ระบบเป็น "คลังความรู้สนับสนุนงาน กสม. ทุกมิติ" — เลือกเริ่ม 2 เรื่องจากตัวเลือกที่เสนอ:
(1) ขยายคลังให้ครบทุกหมวดเอกสาร (2) เพิ่มความแม่นยำ/ลด hallucination ของ AI ทำรอบนี้ 2 อย่าง
พร้อมกันเพราะเชื่อมกันได้พอดี (ดูหัวข้อถัดไป)

**เติมเนื้อหาจริงเข้า 2 ใน 5 หมวดที่เคยว่าง:**
- `06 กฎหมายไทย (Thai Law)/` — 5 ฉบับที่เกี่ยวข้องกับภารกิจ กสม. โดยตรงที่สุด: รัฐธรรมนูญ 2560
  (หมวด 3 + มาตรา 247), พ.ร.ป. กสม. 2560 (มาตรา 26(3) ประกอบมาตรา 42 — ตรงกับที่ skill
  `nhrc-pm-letter` อ้างอิงอยู่แล้ว), พ.ร.บ. ป้องกันและปราบปรามการทรมานฯ 2565, พ.ร.บ. ความเท่าเทียม
  ระหว่างเพศ 2558, พ.ร.บ. ส่งเสริมและพัฒนาคุณภาพชีวิตคนพิการ 2550
- `07 กฎหมายสิทธิมนุษยชนระหว่างประเทศ (International HR Law)/` — 9 ตราสารหลักที่ไทยเป็นภาคี:
  UDHR, ICCPR, ICESCR, CERD, CEDAW, CAT, CRC, CRPD, ICPPED (วันที่ภาคยานุวัติ/มีผลบังคับใช้ของ
  ICPPED — 14 พ.ค. 2567 / 13 มิ.ย. 2567 — ตรวจสอบผ่านเว็บกับแหล่งข้อมูล UN Thailand แล้ว)

ทั้ง 14 ไฟล์เขียนโดย Claude จากความรู้ทั่วไป **ไม่ใช่ทุกจุดตรวจสอบผ่านแหล่งทางการ** — ทุกไฟล์มี
หัวข้อ `## หมายเหตุความถูกต้อง` ท้ายไฟล์ระบุจุดที่ยังไม่ผ่านการตรวจทาน (ส่วนใหญ่คือวันที่แน่นอน
วัน-เดือน-ปี และเลขมาตราย่อยในรายละเอียด) พร้อมลิงก์ไปแหล่งทางการจริง (OHCHR/krisdika.go.th/
ราชกิจจานุเบกษา) ใน `## ลิงก์เอกสารฉบับเต็ม` — **ควรให้เจ้าหน้าที่ตรวจทานก่อนอ้างอิงในรายงานทางการ**
เช่นเดียวกับที่ทำกับ AI legal-refs อยู่แล้ว

**อีก 3 หมวด (08 คลังความรู้, 09/10 คำพิพากษาศาลไทย/ต่างประเทศ) ยังไม่เติม** เพราะคำพิพากษาต้อง
อ้างเลขคดี/วันที่/คำวินิจฉัยที่แม่นยำเป๊ะ (เสี่ยง hallucination สูงกว่ามากถ้าไม่มีคดีจริงอ้างอิง)
และ "คลังความรู้" เป็นหมวดเปิดกว้างที่ควรให้ผู้ใช้ช่วยกำหนดขอบเขตก่อน — รอทิศทาง/แหล่งข้อมูลที่
ตรวจสอบได้ชัดเจนกว่านี้ในรอบถัดไป

**Ground `internationalInstruments` ด้วยคลังใหม่ (แก้จุดอ่อน AI accuracy ที่ใหญ่ที่สุดของ
legal-refs.ts):** เดิม `internationalInstruments` เป็น pure LLM recall ล้วน ไม่มีการ ground เลย
(ต่างจาก `thaiLaws` ที่มี `groundedThaiLaws` จาก OpenThai 2.0 Legal เสริมอยู่แล้ว) พอมีคลังตราสาร
ระหว่างประเทศ/กฎหมายไทยจริงในระบบแล้ว เพิ่มฟิลด์ใหม่ `groundedInternationalInstruments` และ
`groundedThaiLawDocs` ใน `LegalRefsResult` (`web/src/lib/nhrc/legal-refs.ts`) โดยเรียก
`repo.findRelevantCases(content, 5, { category: "..." })` ที่มีอยู่แล้ว (เดิมใช้ทำ "เอกสารที่
เกี่ยวข้อง" — keyword-overlap ranking, คืนค่าว่างถ้าไม่พบจริง ไม่มี fallback เดาสุ่ม) — pattern
เดียวกับ `groundedThaiLaws` ทุกประการ (additive ไม่แทนที่ AI-recall list) ต่างที่จุดเดียว: รายการ
พวกนี้มีหน้าเอกสารจริงในคลังให้ลิงก์ได้ (`groundedThaiLaws` จาก OpenThai เป็น plain text เพราะ
ไม่มีหน้าให้ลิงก์) เลยเรนเดอร์เป็น `<Link>` คลิกได้จริงใน `LegalRefsBox`
(`web/src/app/case/[id]/page.tsx`) แทน `<div>` — เพิ่ม CSS `.cw-legal-ref-item--link` ใน
`chat-workspace.css` ให้ (`<Link>` เป็น inline โดย default ต้องบังคับ `display: block`)

ทดสอบแล้วที่กรณี `103-2564` (จนท.รัฐซ้อมทรมานให้รับสารภาพ): sidebar ขึ้นบล็อกใหม่
"ยืนยันจากคลังตราสารระหว่างประเทศในระบบ" (จับคู่ CAT, ICCPR, ICPPED ฯลฯ จากคำสำคัญ) และ
"ยืนยันจากคลังกฎหมายไทยในระบบ" (จับคู่ พ.ร.บ. ป้องกันและปราบปรามการทรมานฯ) เป็นลิงก์คลิกได้จริง
ไปหน้าเอกสารนั้น ๆ

**ต่อยอด (ยังวันเดียวกัน): เติม "เอกสารตีความ" เข้าหมวด 07 — รอบแรก 10 ฉบับ** — ชื่อหมวดในระบบ
คือ "กฎหมายสิทธิมนุษยชนระหว่างประเทศ**และเอกสารตีความ**" แต่รอบแรกทำแค่ตัวสนธิสัญญา ยังไม่มี
"เอกสารตีความ" เลย เพิ่ม **ความเห็นทั่วไป/ข้อเสนอแนะทั่วไป (General Comments/Recommendations)**
ของกลไกตามสนธิสัญญา 10 ฉบับแรกในโฟลเดอร์ย่อยใหม่ `07 .../ความเห็นทั่วไปและข้อเสนอแนะทั่วไป
(General Comments)/` ตามความเกี่ยวข้องกับประเด็นที่ กสม. เจอบ่อย

**ต่อยอดรอบที่ 2 (7 ส.ค. 2569 ต่อเนื่อง): ผู้ใช้ขอ "ทุกฉบับในทุกสนธิสัญญา"** — สำรวจแล้วพบว่า
สเกลใหญ่กว่าที่คิดมาก (~180 ฉบับรวมทั้ง 7 คณะกรรมการ: HRC/ICCPR, CESCR/ICESCR, CEDAW, CRC,
CERD, CRPD, CAT) ผู้ใช้ยืนยันให้ทำแบบละเอียดเต็มครบทุกฉบับแม้จะใช้เวลาหลายรอบ — ทำโดยส่ง
background agent (Task/Agent tool) แยกตัวต่อคณะกรรมการ ให้แต่ละตัวค้นหารายชื่อ/ปี/หัวข้อจริงผ่านเว็บ
(หลีกเลี่ยง ohchr.org เพราะบล็อก automated fetch ด้วย 403 — ใช้ refworld.org, digitallibrary.un.org,
hrlibrary.umn.edu/gencomm/ แทน) แล้วเขียนไฟล์ตาม pattern เดิมทุกไฟล์ (caveat + ลิงก์อ้างอิง)

**ผลลัพธ์สุดท้าย: ครบทั้ง 7 คณะกรรมการ รวม 179 เอกสารตีความ** (+ 9 ตราสารเดิม = 188 เอกสารในหมวดนี้):
- HRC (ICCPR): GC1-37 ครบ (37 ฉบับ)
- CESCR (ICESCR): GC1-27 ครบ (27 ฉบับ)
- CEDAW: GR1-39 ครบ (38 ไฟล์ + GR31 เป็นฉบับร่วมกับ CRC เก็บไว้ในไฟล์ CRC GC18 แทน)
- CRC: GC1-26 ครบ (26 ฉบับ, รวม GC22/23 ที่เป็นฉบับร่วมกับ CMW)
- CERD: GR1-39 ครบ (39 ฉบับ — agent เจอเพิ่มจากที่คาดไว้ ~36 เป็น 39 เพราะพบ GR16-18, GR22, GR28
  ที่ไม่อยู่ใน candidate list เดิม และ GR38/39 ที่เพิ่งประกาศ ธ.ค. 2568)
- CRPD: GC1-8 ครบ (8 ฉบับ — agent ตรวจแล้วว่า GC9 ยังอยู่ระหว่างร่าง ยังไม่ประกาศใช้ จึงข้ามอย่าง
  ถูกต้อง ไม่ได้แต่งขึ้นมาเอง)
- CAT: GC1-4 ครบ (4 ฉบับ — ครบเท่าที่มีจริง)

**บั๊กที่เจอและแก้ระหว่างทำ:**
1. **Windows MAX_PATH (260 ตัวอักษร) ทำให้เขียน/อ่านไฟล์ไม่ได้แบบเงียบ ๆ** — โฟลเดอร์ย่อยนี้อยู่ลึก
   (path prefix ยาว 146 ตัวอักษรก่อนถึงชื่อไฟล์) พอรวมกับชื่อไฟล์ที่บรรยายหัวข้อเต็ม ๆ ตาม pattern เดิม
   ทำให้บาง path ยาวเกิน 260 ตัวอักษร — `open()` ธรรมดาของ Python **จะ throw `FileNotFoundError`
   แบบไม่มีคำเตือน** (ไม่ใช่ error ที่ `_parse_generic`'s `except UnicodeDecodeError` จะดักได้ ถ้าไม่แก้
   `setup_obsidian_index.py` จะ crash ตอน reindex) พบ 25 ไฟล์แบบนี้ (+ 3 ไฟล์ที่ agent ตัวหนึ่งเข้าใจผิด
   ว่าเขียนไม่สำเร็จแล้วเขียนซ้ำเป็นชื่อสั้นกว่า ทำให้มี "เลขที่" ซ้ำ 3 คู่ ต้องลบไฟล์ชื่อยาวทิ้ง) แก้โดย
   rename ไฟล์ทั้ง 25 ให้ชื่อสั้นลง (ใช้ extended-length path prefix `\\?\` ตอน rename เพราะ path เดิม
   ยาวเกินจน `os.rename` ธรรมดาก็เข้าไม่ถึง) เนื้อหาข้างในไฟล์ (หัวข้อเต็ม, เนื้อหา) **ไม่กระทบ** เพราะ
   ชื่อเรื่องที่แสดงบนเว็บดึงจาก heading `# ...` ในไฟล์ ไม่ใช่จากชื่อไฟล์ — เหมือน pattern เดิมที่มีอยู่แล้ว
   ในระบบสำหรับเอกสารงานวิจัยที่ชื่อไฟล์ถูกตัดสั้น (ดูหัวข้อ "งานวิจัย" ด้านบน) ตรวจสอบหลังแก้แล้วว่า
   ทั้ง 179 ไฟล์เปิดด้วย Python `open()` ธรรมดาได้หมด ไม่มี path ไหนยาวเกิน 259 ตัวอักษรอีก
2. Session/usage limit ของ background agent หมดกลางทาง (ข้อความ "You've hit your session limit")
   หลายรอบระหว่างทำงานคู่ขนาน 6-7 agent พร้อมกัน — แก้โดยรอ reset แล้วส่ง agent ไปทำต่อเฉพาะเลขที่
   ยังขาด (เช็คไฟล์บนดิสก์จริงก่อนทุกครั้งเพื่อไม่ให้ agent ทำซ้ำ)

Reindex สุดท้ายผ่านไม่มี error, ทดสอบใน browser แล้วว่าเอกสารที่ชื่อไฟล์ถูกย่อ (เช่น CRC GC8, CERD
GR39) เรนเดอร์เนื้อหาเต็มถูกต้อง ไม่มี error ใน console

**บั๊กที่เจอระหว่างทำ (แก้แล้ว):** `splitGeneralSections`'s parser split เนื้อหาที่ตำแหน่ง
`\n(?=##\s)` เท่านั้น — ข้อความท้ายไฟล์ที่ไม่ได้อยู่ใต้หัวข้อ `##` (ตอนแรกใช้ `---` + blockquote
สำหรับหมายเหตุความถูกต้อง) เลยไปเกาะติดท้าย section สุดท้ายในไฟล์แทนที่จะแยกเป็นของตัวเอง แก้โดย
เปลี่ยนเป็นหัวข้อ `## หมายเหตุความถูกต้อง` จริง ๆ แล้วเพิ่มเข้า `GENERAL_SECTION_ORDER` ให้อยู่
ลำดับสุดท้ายเสมอ (พร้อมเพิ่ม `มาตราสำคัญ`/`พันธกรณีของไทย`/`ความเชื่อมโยงกับกรณีตรวจสอบของ กสม.`
เข้า order เดิมด้วย เพราะ headers พวกนี้เป็นของใหม่ที่ template เดิมไม่มี ไม่งั้นจะโดน sort
ไปแทรกก่อน "ประเด็นสิทธิที่เกี่ยวข้อง" อย่างไม่ตั้งใจ — JS `Array.sort` เป็น stable sort)

## แก้บั๊ก semantic search + ปรับ Ask NHRC ให้ตอบครบทุกหมวดและมีโครงสร้าง (9 ส.ค. 2569)

ผู้ใช้ทดสอบถามใน Ask NHRC ว่า "ขอแนวทางการตรวจสอบกรณีที่ดิน" แล้วได้แต่งานวิจัยกลับมา
ทั้งที่มีกรณีตรวจสอบที่ดินจริง 151 กรณีและตราสารระหว่างประเทศที่เกี่ยวข้องอยู่ในคลัง —
สืบสาเหตุพบ 3 ปัญหาซ้อนกัน:

1. **Embeddings ไม่ครบ**: ตาราง Supabase `nhrc_embeddings` มี 430/623 รายการ — 193 รายการที่ขาด
   ตรงกับเอกสารหมวด 06/07 ที่เพิ่งเพิ่มเข้าไปทั้งหมด (ไม่เคยรัน `embed-nhrc-documents.mjs` ซ้ำหลัง
   เพิ่มเนื้อหา) แก้โดยรันสคริปต์ backfill จนครบ 623/623
2. **บั๊กจริงใน `semantic-search.ts`**: ฟังก์ชัน cast ผลลัพธ์จาก Supabase RPC (`match_nhrc_documents`
   คืนคอลัมน์ `document_id` แบบ snake_case) ตรงๆ เป็น type `SemanticMatch` ที่ประกาศฟิลด์เป็น
   `documentId` แบบ camelCase โดยไม่ได้ map จริง ทำให้ `m.documentId` เป็น `undefined` เสมอ แล้วไป
   ชน `repository.ts`'s `getCaseById(undefined)` ที่ดันแมตช์กับ "เอกสารตัวแรกที่ไม่มี case_id" เพราะ
   JS เช็ค `undefined === undefined` เป็นจริง — **ผลคือ semantic search ล้มเหลวเงียบๆ มาตลอด ทุก
   คำตอบวิ่งไปใช้ keyword-substring fallback (`findRelevantCases`) โดยไม่มีใครรู้** ซึ่งอธิบายได้ว่าทำไม
   คำถามเรื่องที่ดินไปแมตช์กับงานวิจัยที่ชื่อขึ้นต้นด้วย "แนวทางการ..." (ตรงตัวอักษรกับคำถาม) แทนที่จะ
   เป็นกรณีตรวจสอบจริง แก้โดยเพิ่ม mapping `{document_id} -> {documentId}` ให้ถูกต้องใน
   `semantic-search.ts` ทดสอบยันด้วยการยิง query ตรงไปที่ Supabase RPC เอง เทียบ before/after
3. **ไม่มีการกระจายหมวดหมู่**: แม้ semantic search ทำงานถูกแล้ว การตัด top-N ตาม cosine similarity
   ล้วนๆ ก็ยังเสี่ยงได้ผลจากหมวดเดียว (เช่น กรณีตรวจสอบที่ดิน 151 กรณีแย่งอันดับกันเองจนกลบ
   ตราสารระหว่างประเทศที่เกี่ยวข้อง) แก้โดยเพิ่ม diversification ใน `findRelevantDocuments()`
   (`api/ask-nhrc/route.ts`): ดึง pool กว้างขึ้น (match_count 60), จำกัดไม่เกิน 3 รายการต่อหมวดหมู่
   (ตัดหมวดที่ similarity ต่ำกว่า top-0.12 หรือต่ำกว่า 0.55 ทิ้ง), รวมได้สูงสุด 10 หลักฐาน — ใช้เฉพาะ
   ตอนผู้ใช้ไม่ได้ระบุหมวดหมู่ผ่าน filter ด้านซ้ายเอง (ถ้าระบุแล้วเคารพ filter เดิม ไม่ diversify ทับ)

ต่อยอดอีกเรื่อง — ผู้ใช้ขอให้ "การตอบต้องเรียบเรียงให้ดี" โดยยกตัวอย่าง fourcorners.law (แพลตฟอร์ม
ถาม-ตอบกฎหมายไทย) เป็นต้นแบบ: เปิดด้วยสรุปสั้น, แบ่งเป็นหัวข้อย่อยตามแหล่งอ้างอิงแต่ละชิ้น, มี
`แหล่งที่มา: [n]` กำกับ, ตามด้วยคำพูดยกมาสั้นๆ + การตีความ + bullet list ปรับระบบตาม:

- **Prompt**: เขียน system prompt ใหม่บังคับโครงสร้าง Markdown ("## " ต่อประเด็น + "**แหล่งที่มา:**
  [n]" + bullet list + ปิดท้ายด้วย "## สรุปแนวทาง") และเพิ่ม `max_tokens` เป็น 2048 (จากเดิม 1024
  ที่ไม่พอสำหรับคำตอบหลายหัวข้อ)
- **Renderer ใหม่** `web/src/components/markdown-lite.tsx` — ตัว parser Markdown แบบเบา เขียนเอง
  ไม่พึ่ง dependency ใหม่ (`react-markdown` ฯลฯ) รองรับแค่ subset ที่ prompt ควบคุมเอง: heading
  (`#`-`######`, ทนกรณีโมเดลใช้ `#` เดี่ยวแทน `##` ตามสั่ง), bullet (`- `), blockquote (`> `),
  `**bold**`, และ `[n]` แปลงเป็น `<sup>` คลิกได้
- **`nhrc-workspace.tsx`**: เปลี่ยนฝั่ง AI message จาก `{chat.text}` ดิบๆ เป็น
  `<MarkdownLite text={chat.text} onCiteClick={scrollToCitation} />` — คลิก `[n]` แล้ว scroll ไปหา
  การ์ดอ้างอิงที่ตรงกันในแผงขวา (ใส่ `id="cw-ref-{n}"`) พร้อม highlight ชั่วคราว 2 วิ
- **`evidenceAnswer()`** (fallback ตอนไม่มี AI/AI ล้มเหลว) ปรับให้ group ตามหมวดหมู่ด้วย "## " header
  เดียวกัน จะได้ผ่าน renderer เดียวกันแล้วดูเป็นระเบียบแม้ไม่มี AI

ทดสอบจริงผ่านเบราว์เซอร์ (ไม่ใช่แค่ curl) ด้วยคำถาม "สิทธิในที่ดินทำกินของกลุ่มชาติพันธุ์มีหลักกฎหมาย
อะไรรองรับบ้าง" — คำตอบออกมาครอบคลุม 4 หมวดในคำตอบเดียว (มาตรฐานระหว่างประเทศ: CERD GR23 +
CESCR GC26, กฎหมายไทยใหม่: พ.ร.บ. คุ้มครองวิถีชีวิตกลุ่มชาติพันธุ์ 2568, กรณีตรวจสอบจริงของ กสม.:
บ้านพะเด๊ะ/ภูทับเบิก, งานวิจัย) แบ่งเป็นหัวข้อจริงตาม DOM (`heading` element ไม่ใช่ข้อความ `#` ดิบ),
`[n]` เรนเดอร์เป็นปุ่มคลิกได้จริง ไม่มี error ใน console, `tsc --noEmit` ผ่าน
